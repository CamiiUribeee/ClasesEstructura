...
public class Pila<T> {
    ...
    public void irAdelante(T elemento){
        JOptionPane.showInputDialog("Inserta las paginas de navegacion");
        if (tope == elementos.length - 1) {
            aumentarCapacidad();
        }
        elementos[++tope] = elemento;
    ...
    public void historialPaginas() {
        if (isEmpty()) {
            JOptionPane.showMessageDialog(null, "La lista esta vacia");
            return;
        }
    
        JOptionPane.showMessageDialog(null, "Paginas de navegacion: ");
        for (int i = tope; i >= 0; i--) {
            JOptionPane.showMessageDialog(null, elementos[i]);
        }
    }
